import { LocalVideoTrack, Room } from 'twilio-video-ssr';
    ...
    if (!isPublishing) {
      if (videoTrack) {
        ...
        const localTrackPublication =
          localParticipant && localParticipant.unpublishTrack(videoTrack);
        ...
        getLocalVideoTrack({ deviceId: { exact: previousDeviceIdRef.current } })
          .then(
            track => localParticipant && localParticipant.publishTrack(track, { priority: 'low' })
          )
          .catch(onError)
